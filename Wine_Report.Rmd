...
wine.reparam <- wine %>%
  mutate(bound.sulfur.dioxide = total.sulfur.dioxide - free.sulfur.dioxide,
         total.acidity = fixed.acidity + volatile.acidity + citric.acid - pH,
         alc.density = density * alcohol)
```

```{r}
reparam.wine.lm <- lm(quality ~ total.acidity + residual.sugar + chlorides + free.sulfur.dioxide + bound.sulfur.dioxide + alcohol + density + sulphates + color, data = wine.reparam)
vif(reparam.wine.lm)
